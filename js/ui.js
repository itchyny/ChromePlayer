...
window.UI = {
  ...
  initbuttons: function () {
    ...
    this.div.open.click(function () { self.div.fileselect.click(); });
    this.div.fileselect.change(function (e) { player.readFiles(e.target.files); });
    this.div.play.click(function () { player.toggle (); });  // <- audio.play
    this.div.pause.click(function () { player.toggle (); });  // <- audio.pause
    this.div.prev.click(function () { player.prev (); });
    this.div.next.click(function () { player.next (); });
    this.div.mute.click(function () { player.mute (); });
    this.div.volumeon.click(function () { player.resume (); });
    this.div.repeat.click(function () { player.repeatToggle (); });
    this.div.shuffle.click(function () { player.toggleshuffle (); });
    this.div.conf.click(function () { self.div.config.fadeToggle(200); });
    $("img.lbutton, img.rbutton")
    .mouseup(function (e) { $(this).css({ "top": "50%" }); })
    .mousedown(function (e) { console.log($(this).css("top")); $(this).css({ "top": parseInt($(this).css("top"), 10) * 1.03 + "%" }); });
  },
  ...
  initsplitter: function () {
    ...
    for(var thname in w) {
      $("th." + thname)
        .width(w[thname] * (this.div.thead.width()));
    };
  },
