    ...
    $.fn.drag_drop_multi_select = function(settings){
        ...
        return this.each(function(i) {
            ...
            $self.find(config.element_to_drag_drop_select).each(function(){
                ...
                // BIND CLICK ITEM
                $item.bind('click.ddms_item_clicked',function(e){      // EDITTED BY itchyny
                    $('.last-select')      // EDITTED BY itchyny
                      .removeClass('last-select');      // EDITTED BY itchyny
                    if(e.ctrlKey/*$.fn.keyisPressed(CTRL_KEY)|| $.fn.keyisPressed(SHIFT_KEY)||($.fn.drag_drop_multi_select.stack[$.fn.drag_drop_multi_select.get_instance_id($(this).attr('ddms'))].selected.length == 1 && $item.hasClass($.fn.drag_drop_multi_select.settings[$.fn.drag_drop_multi_select.get_instance_id($item.attr('ddms'))].selectedClass))*/){      // EDITTED BY itchyny
                ...
                $item.bind('ddms.select',function(){
                    ...
                    $(this).addClass('last-select');      // EDITTED BY itchyny
